...
public static class AsmLowerer
{
    ...
    private static void LowerCallExtractFlag(List<AsmNode> nodes, LirOpInstruction op, bool isC)
    {
        ...
        nodes.Add(Emit(P2Mnemonic.BITL, destReg, new AsmImmediateOperand(0)));
        if (isC)
        {
            nodes.Add(Emit(P2Mnemonic.BITC, destReg, new AsmImmediateOperand(0)));
        }
        else
        {
            nodes.Add(Emit(P2Mnemonic.BITZ, destReg, new AsmImmediateOperand(0)));
        }
    }
