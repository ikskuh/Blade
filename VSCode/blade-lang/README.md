...
- `Blade: Open Preview To The Side` command for compiling the active Blade document and showing the compiler's HTML report in a side preview.
...
```text
<blade.path or blade> --report html,-
```

The document source is piped to stdin. The preview uses the compiler's emitted HTML report directly instead of reconstructing the report inside the extension.
